...
function loadStatsPage()
{
	...
	{
		buffScore = table.rows[row_counter].cells[2].innerHTML;
		oppScore = table.rows[row_counter].cells[3].innerHTML;
		diff = buffScore - oppScore;
		if (diff > 0)
		{
			  buffWin++;
...
// }
				...
				// </script>
					...
					function loadPlayersPage()
					{
						...
						var size = players.length; //how many players there are
						for(let i = 0; i < size; i++)
						{
							var name = players[i].name; //will show name of player in dropdown
							select_player.innerHTML += "<a class=\"dropdown-item\" href=\"#\" onclick=\"switchPlayers("+i+")\">" + name + "</a>";
							//creates an anchor tag with href set to # and make an onlick aka button that will switch through the players depending on the number called and will show their name
						}
					}
					...
					function switchPlayers(playerNum)
					{
						...
						var avg_pass = parseInt(players[playerNum].pass_yards / players[playerNum].games_played);
						var avg_rush = parseInt(players[playerNum].rushing_yards / players[playerNum].games_played);
						var avg_receive = parseInt(players[playerNum].receiving_yards / players[playerNum].games_played);
						//parseInt goes into what's already in the table and then I calculate the avg using the given numbers

						document.getElementById("avg_p_yards").innerHTML = avg_pass;
						document.getElementById("avg_r_yards").innerHTML = avg_rush;
						document.getElementById("avg_rec_yards").innerHTML = avg_receive;
					}
